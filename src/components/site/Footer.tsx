    ...
    <footer style={{ background: "var(--text)", color: "white" }}>
      <div className="max-w-7xl mx-auto px-6 py-16 grid gap-12 md:grid-cols-12">
        <div className="md:col-span-4">
          <h2 className="font-display text-3xl font-black tracking-tight text-white">Inspired Institute</h2>
          <p className="font-semibold mt-1 text-base italic" style={{ color: "var(--accent)" }}>Empowering Future Scientists</p>
        ...
        <div className="md:col-span-3">
          ...
          <ul className="space-y-3">
            {courseLinks.map((item, i) => (
              <li key={item.label + i}>
                <Link to={item.href} className="flex items-center gap-2 text-sm text-white/60 hover:text-white transition-colors">
        ...
        <div className="md:col-span-3">
          ...
          <div className="relative mb-8">
            ...
            />
            <button className="absolute right-2 top-1/2 -translate-y-1/2 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition-colors" style={{ background: "var(--accent)" }}>→</button>
          </div>
      ...
      <div className="border-t border-white/10 py-5">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-3 text-xs text-white/40">
          <div className="flex gap-5">
            ...
            <Link to="/Connect" className="hover:text-white/70 transition-colors">Contact</Link>
          </div>
