...
/* ── DATA ── */
const testimonials = [
  { name: "Rohan Mehta",   role: "JEE Advanced · AIR 280", stars: 5, text: "The concept-first teaching at Inspired Institute changed how I approach every problem. I cleared JEE Advanced with confidence I never thought I'd have.", avatar: "https://i.pravatar.cc/160?img=12" },
  { name: "Priya Shah",    role: "NEET · Score 688/720",   stars: 5, text: "My biology scores went from average to exceptional in just one semester. The personal mentorship here is truly unmatched.",                                avatar: "https://i.pravatar.cc/160?img=47" },
  { name: "Kiran Patel",   role: "Class 10 · Board 97.4%", stars: 5, text: "Foundation program built my confidence from the ground up. I went from dreading maths to actually enjoying problem-solving sessions.",                     avatar: "https://i.pravatar.cc/160?img=33" },
  { name: "Anjali Desai",  role: "Parent · Class 11",      stars: 5, text: "Weekly test reports and one-on-one sessions gave us clarity on where to focus. My daughter improved 30% in three months.",                                  avatar: "https://i.pravatar.cc/160?img=45" },
  { name: "Aarav Sinha",   role: "JEE Main · 99.6 %ile",   stars: 5, text: "The mock test culture and post-test analysis sessions are world class. I always knew exactly what to fix next.",                                            avatar: "https://i.pravatar.cc/160?img=15" },
];
  ...
  return (
    <main style={{ background: "var(--bg)" }}>
      ...
      <section className="relative py-24 px-6 overflow-hidden"
        style={{background:"linear-gradient(135deg, #2E285C 0%, #342C68 50%, #4C438A 100%)"}}>
        {/* ambient glow blobs */}
        <div className="absolute -top-32 -left-20 w-[420px] h-[420px] rounded-full blur-3xl opacity-30"
          style={{background:"radial-gradient(circle, var(--accent) 0%, transparent 70%)"}}/>
        <div className="absolute -bottom-32 -right-10 w-[460px] h-[460px] rounded-full blur-3xl opacity-25"
          style={{background:"radial-gradient(circle, #8B7CFF 0%, transparent 70%)"}}/>

        <div className="relative max-w-6xl mx-auto">
          <FadeIn>
            <div className="text-center mb-14">
              <span className="inline-block text-[0.7rem] font-extrabold tracking-[0.2em] uppercase px-3.5 py-1.5 rounded-full"
                style={{background:"rgba(255,255,255,0.1)",color:"#fff",border:"1px solid rgba(255,255,255,0.18)",backdropFilter:"blur(8px)"}}>
                Testimonials
              </span>
              <h2 className="font-display text-4xl md:text-5xl font-black mt-5 text-white">
                Loved by Students &<br/>
                <span style={{background:"linear-gradient(135deg, var(--accent) 0%, #ffd9b3 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>
                  Their Families
                </span>
              </h2>
              <p className="mt-4 text-base md:text-lg max-w-xl mx-auto" style={{color:"rgba(255,255,255,0.7)"}}>
                Real stories from the Inspired Institute community.
              </p>
            </div>
          </FadeIn>
          ...
          <div className="relative" style={{minHeight:"360px"}}>
            {testimonials.map((t,i)=>{
              ...
              const opacity = isActive ? 1 : visible ? 0.35 : 0;
              ...
              return (
                <div key={t.name}
                  ...
                  <div className="w-full max-w-2xl p-8 md:p-10 rounded-3xl relative"
                    style={{
                    ...
                    <div className="absolute top-0 left-0 w-32 h-32 rounded-tl-3xl pointer-events-none"
                      style={{background:"linear-gradient(135deg, rgba(255,255,255,0.18), transparent 70%)"}}/>

                    <div className="flex gap-1 mb-5">
                      {Array.from({length:t.stars}).map((_,si)=>(
                        <Star key={si} size={18} fill="var(--accent)" stroke="none"/>
                      ))}
                    </div>

                    <p className="font-display text-xl md:text-2xl leading-relaxed text-white font-medium mb-8">
                      "{t.text}"
                    </p>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        ...
                        <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center"
                          style={{background:"var(--accent)",border:"2px solid #2E285C"}}>
                          <Star size={9} fill="#fff" stroke="none"/>
                        </div>
                      </div>
                      <div>
                        <p className="font-bold text-white text-base">{t.name}</p>
                        <p className="text-sm" style={{color:"rgba(255,255,255,0.65)"}}>{t.role}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex gap-2 mx-2">
              {testimonials.map((_,i)=>(
                <button key={i} onClick={()=>setActiveTestimonial(i)}
                  ...
                  style={{
                    width:i===activeTestimonial?28:8, height:8,
                    background:i===activeTestimonial?"var(--accent)":"rgba(255,255,255,0.3)",
                  }}
